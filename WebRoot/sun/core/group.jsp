...
<script type="text/javascript" charset="utf-8"><!--
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'groupAction!datagrid.action',
			title : '组织机构列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			...
			columns : [ [ 
				...
				/**
						...
						*/
				},{field:'obid',title:'组织代码',align:'center',sortable:true,width:240,
						/**
						editor : {
							type : 'validatebox',
				...
				},{field:'cname',title:'组织名称',align:'center',sortable:false,width : 120,
			  	    editor : {
							type : 'validatebox',
			   ...
			   {field:'cdesc',title:'组织描述',align:'center',sortable:false,width : 160,
			  	    editor : {
							type : 'validatebox',
							options : {required:true}
					},
			   ...
			   {field:'pobid',title:'上级组织',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'combobox',
							options : {
								url : 'groupAction!combox.action',
								valueField : 'obid',
								textField : 'cname'
							}
					},
				...
				},{field:'pcname',title:'上级组织',align:'center',sortable:false,hidden:true,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {required:false}
					},
			...
			onAfterEdit : function(rowIndex, rowData, changes) {
				...
				if (inserted.length > 0) {
					url = 'groupAction!add.action';
				}
				if (updated.length > 0) {
					url = 'groupAction!edit.action';
				}
	...
	function _search() {
		datagrid.datagrid('load', jeecg.serializeObject(searchForm));
	}
