...
import sun.service.core.GroupServiceI;
...
public class GroupServiceImpl extends BaseServiceImpl implements GroupServiceI {
	...
	private BaseDaoI<GroupEntity> groupEntityDao;

	public BaseDaoI<GroupEntity> getGroupEntityDao() {
		return groupEntityDao;
	}
	@Autowired
	public void setGroupEntityDao(BaseDaoI<GroupEntity> groupEntityDao) {
	...
	private String addWhere(GroupPage groupPage, String hql, List<Object> values) {
		...
		SearchSqlGenerateUtil.createSearchParamsHql(hqlbf, values, groupEntity);
		hql = hqlbf.toString();
		//-----------------------------------------------------
	...
	public void update(GroupPage groupPage) throws Exception {
	    ...
	    if(t != null) {
			...
			MyBeanUtils.copyBeanNotNull2Bean(groupPage, t);
		}
	}
