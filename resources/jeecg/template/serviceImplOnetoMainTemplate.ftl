...
public class ${entityName}ServiceImpl extends BaseServiceImpl implements ${entityName}ServiceI {
	...
	public void add(${entityName}Page ${entityName?uncap_first}Page) {
		if (${entityName?uncap_first}Page.get${jeecg_table_id?cap_first}() == null || ${entityName?uncap_first}Page.get${jeecg_table_id?cap_first}().trim().equals("")) {
			${entityName?uncap_first}Page.set${jeecg_table_id?cap_first}(UUID.randomUUID().toString());
		}
	...
	public void addMain(${entityName}Page ${entityName?uncap_first}Page,<#list subTab as sub>List<${sub.entityName}Page> ${sub.entityName?uncap_first}List<#if sub_has_next>,</#if></#list>)  throws Exception{
		...
		for(${sub.entityName}Page page:${sub.entityName?uncap_first}List){
			...
			${sub.entityName?uncap_first}.set${jeecg_table_id?cap_first}(UUID.randomUUID().toString());
	...
	public void editMain(${entityName}Page ${entityName?uncap_first}Page,<#list subTab as sub>List<${sub.entityName}Page> ${sub.entityName?uncap_first}List<#if sub_has_next>,</#if></#list>)  throws Exception{
	    ...
	    <#list subTab as sub>
		    <#list sub.foreignKeys as key>
		    <#if key?lower_case?index_of("${jeecg_table_id}")!=-1>
		String ${jeecg_table_id}${sub_index} = ${entityName?uncap_first}Page.get${jeecg_table_id?cap_first}();
		    <#else>
		String ${key?uncap_first}${sub_index} = ${entityName?uncap_first}Page.get${key}();
		    </#if>
		    </#list>
	    </#list>
		...
		for(${sub.entityName}Page page:${sub.entityName?uncap_first}List){
			...
			${sub.entityName?uncap_first}Entity.set${jeecg_table_id?cap_first}(UUID.randomUUID().toString());
			//外键设置
			 <#list sub.foreignKeys as key>
			    <#if key?lower_case?index_of("${jeecg_table_id}")!=-1>
			...
			for (String id : ids.split(",")) {
				...
				if (t != null) {
					${entityName?uncap_first}EntityDao.delete(t);
					    ...
					    <#list sub.foreignKeys as key>
					    <#if key?lower_case?index_of("${jeecg_table_id}")!=-1>
					    String ${jeecg_table_id}${sub_index} = t.get${jeecg_table_id?cap_first}();
					    <#else>
					    String ${key?uncap_first}${sub_index} = t.get${key}();
					    </#if>
					    </#list>
					    </#list>
					   <#list subTab as sub>
	...
	public ${entityName}Entity get(String ${jeecg_table_id}) {
	...
	public List<${sub.entityName}Page> get${sub.entityName}ListByFkey(<#list sub.foreignKeys as key><#if key?lower_case?index_of("${jeecg_table_id}")!=-1>String ${jeecg_table_id}<#else>String ${key?uncap_first}</#if><#if key_has_next>,</#if></#list>) {
